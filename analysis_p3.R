...
mm = log2(plotCountMat[,a]+1) - log2(plotCountMat[,b]+1)
aa = log2(plotCountMat[,a]+1) + log2(plotCountMat[,b]+1)
...
for (pool in mypools){
  ...
  ##barplot
  tiff(filename=paste("./figures/boxplots/","boxplot_p3_", pool, ".tiff", seIp=""),
       width = 5, height = 5, units = 'in', res = 200)
  ...
  boxplot( plotCountMat, main=pool, col=c(rep(2,8),rep(1,8)), ylab="log2(counts per million)", xlab="Sample Number")
  ...
  # 
  #colnames(myCountsMat) = currSampleInfo[,3]
  trSel = which(currSampleInfo[,3]=="VEM")
  conSel = which(currSampleInfo[,3]=="VEH")
...
finalOut = cbind(tag=rownames(finalOut), finalOut[, c(3,1,2,4)])

write.table(finalOut, file="gene_list_P3_17Sep17.txt", sep="\t"
            ,col.names=T, row.names=F, quote=F )


  ##as.character(tppGenes[,1])%in%as.character(topPvalsNamed)
# myGenes[,3] = as.character(myGenes[,3])
# # qq = data.frame(table(myGenes[,3]))
# 
# geness = unique(myGenes[,3])
#   
# finalOut = c()
# 
# for (gene in geness){
...
# write.table(finalOutMat, file="gene_list_11Sep17.txt", sep="\t"
